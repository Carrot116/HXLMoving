...
+ (CustomOverlay*)customWithPointArray:(NSArray*)points{
    CustomOverlay* polyline = [[CustomOverlay alloc] initWithPointArray:points ];
    return polyline;
}
...
-(id)initWithPointArray:(NSArray*)points{
    BMKMapPoint* pts = new BMKMapPoint[points.count];
    for (int i = 0;  i < points.count; i++) {
        pts[i].x = ((__bridge BMKMapPoint*)(points[i]))->x;
        pts[i].y = ((__bridge BMKMapPoint*)(points[i]))->y;
    }
    return [self initWithPoints:pts count:points.count];
}
